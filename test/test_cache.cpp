...
TEST_CASE("cache initial state", "[cache]") {
	...
	SECTION("initial state"){
		...
		APS2_STATUS status;
		status = write_memory(ip_addr.c_str(), WFA_OFFSET, test_wf_a.data(), test_wf_a.size() );
		REQUIRE( status == APS2_OK );
		status = write_memory(ip_addr.c_str(), WFB_OFFSET, test_wf_b.data(), test_wf_b.size() );
		REQUIRE( status == APS2_OK );
		...
		status = write_memory(ip_addr.c_str(), SEQ_OFFSET, test_seq.data(), test_seq.size() );
		REQUIRE( status == APS2_OK );
		...
		while (check_addr < 0xc4000000 + (1 << 18)) {
			read_memory(ip_addr.c_str(), check_addr, &*it, 256); //ugly maybe std::pointer_from will exist some day
			std::advance(it, 256);
			check_addr += 256*4;
		}
