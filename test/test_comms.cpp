...
bool test_mem_write_read(uint32_t memStartAddr, uint32_t memHighAddr,
                         size_t testLength, uint32_t alignmentMask) {
  /*
  Helper function to test writing and reading to a range of memory with a
  particular alignment.
  */

  auto test_vec = RandomHelpers::random_data(testLength / 4);
  auto start_addr = RandomHelpers::random_address(
      memStartAddr, memHighAddr - testLength, alignmentMask);

  cout << "writing " << std::dec << testLength / (1 << 10) << " kB starting at "
       << hexn<8> << start_addr << "..... ";
  cout.flush();
  ...
  auto duration =
      std::chrono::duration_cast<std::chrono::microseconds>(stop - start)
          .count();
  cout << "at " << static_cast<double>(testLength) / duration << " MB/s."
       << concol::RESET << endl;

  // Check a few 1kB count entries for correctness
  std::uniform_int_distribution<uint32_t> addr_distribution(
      start_addr, start_addr + testLength - 1024);
  bool passed = true;
...
TEST_CASE("memory writing and reading", "[read_memory,write_memory]") {
  ...
  SECTION("variable write sizes") {
    ...
    for (unsigned writeSize = 0; writeSize <= 256; writeSize += 4) {
      ...
      APS2_STATUS status;
      status = write_memory(ip_addr.c_str(), idx * 4, outData.data() + idx,
                            writeSize);
...
TEST_CASE("sequencer SDRAM write/read", "[sequencer SDRAM]") {
  ...
  SECTION("basic write/read") {
    ...
    get_firmware_version(ip_addr.c_str(), &firmware_version, nullptr, nullptr,
                         nullptr);
    size_t testLength =
        ((firmware_version & 0x00000fff) >= 0x300) ? (1 << 25) : (1 << 21);
    bool passed;
    // sequence
    cout << "seq. memory: ";
    passed = test_mem_write_read(SEQ_OFFSET, SEQ_OFFSET + (1 << 29), testLength,
                                 0xf);
    REQUIRE(passed);
    // waveform
    cout << "wf. memory: ";
    passed = test_mem_write_read(WFA_OFFSET, WFA_OFFSET + (1 << 29), testLength,
                                 0xf);
    REQUIRE(passed);
  }
}
...
TEST_CASE("configuration SDRAM writing and reading", "[configuration SDRAM]") {
  ...
  get_firmware_version(ip_addr.c_str(), &firmware_version, nullptr, nullptr,
                       nullptr);
  size_t testLength =
      ((firmware_version & 0x00000fff) >= 0x300) ? (1 << 22) : (1 << 18);
  // write to configuration memory
  auto test_vec = RandomHelpers::random_data(testLength / 4);
  uint32_t alignment = 0x7; // 8byte
  auto start_addr = RandomHelpers::random_address(0, 0x07ffffff - testLength,
                                                  alignment); // 128MB
  cout << "config. memory: writing " << std::dec << testLength / (1 << 10)
       << " kB starting at " << hexn<8> << start_addr << "..... ";
  cout.flush();
  auto start = std::chrono::steady_clock::now();
  write_configuration_SDRAM(ip_addr.c_str(), start_addr, test_vec.data(),
                            test_vec.size());
  auto stop = std::chrono::steady_clock::now();
  auto duration =
      std::chrono::duration_cast<std::chrono::microseconds>(stop - start)
  ...
  for (size_t ct = 0; ct < 10; ct++) {
    ...
    read_configuration_SDRAM(ip_addr.c_str(), check_addr, check_vec.size(),
                             check_vec.data());
...
TEST_CASE("eprom read/write", "[eprom]") {
  ...
  SECTION("basic write/read") {
    ...
    auto start_addr = RandomHelpers::random_address(
        0x01c00000, 0x01ffffff - test_length, alignment);
    cout << "ERPOM: writing " << std::dec << test_length / (1 << 10)
         << " kB starting at " << hexn<8> << start_addr << "..... ";
    cout.flush();
    auto start = std::chrono::steady_clock::now();
    APS2_STATUS status;
    status = write_flash(ip_addr.c_str(), start_addr, test_vec.data(),
                         test_vec.size());
    REQUIRE(status == APS2_OK);
    auto stop = std::chrono::steady_clock::now();
    auto duration =
        std::chrono::duration_cast<std::chrono::microseconds>(stop - start)
            .count();
    cout << "at " << static_cast<double>(test_length) / duration << " MB/s."
         << concol::RESET << endl;
    ...
    while (check_addr < start_addr + test_length) {
      read_flash(ip_addr.c_str(), check_addr, 256,
                 &*it); // ugly maybe std::pointer_from will exist some day
      std::advance(it, 256);
      check_addr += 256 * 4;
    }
