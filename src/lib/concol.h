...
operator<<(std::basic_ostream<elem, traits> &os, colour col) {
  os.flush();
  settextcolor(col);
  return os;
}
...
operator>>(std::basic_istream<elem, traits> &is, colour col) {
...
// Just use some ASCII escape sequences on Linux.  Most modern terminals should
// support this.
#include <string>
