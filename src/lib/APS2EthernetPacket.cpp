...
APS2EthernetPacket::APS2EthernetPacket()
    : header{{}, {}, APS_PROTO, 0, APS2Command(), 0}, payload(0){};

APS2EthernetPacket::APS2EthernetPacket(
    APS2Command command,
    const uint32_t &addr /*see header for default addr=0 */)
    : header{{}, {}, APS_PROTO, 0, command, addr}, payload(0){};

APS2EthernetPacket::APS2EthernetPacket(const MACAddr &destMAC,
                                       const MACAddr &srcMAC,
                                       APS2Command command,
                                       const uint32_t &addr)
...
APS2EthernetPacket::APS2EthernetPacket(const vector<uint8_t> &packetData) {
  ...
  */
  // Helper function to turn two network bytes into a uint16_t or uint32_t
  // assuming big-endian network byte order
  auto bytes2uint16 = [&packetData](size_t offset) -> uint16_t {
    return (packetData[offset] << 8) + packetData[offset + 1];
  };
  auto bytes2uint32 = [&packetData](size_t offset) -> uint32_t {
    return (packetData[offset] << 24) + (packetData[offset + 1] << 16) +
           (packetData[offset + 2] << 8) + packetData[offset + 3];
  };

  std::copy(packetData.begin(), packetData.begin() + 6,
            header.dest.addr.begin());
  std::copy(packetData.begin() + 6, packetData.begin() + 12,
            header.src.addr.begin());
  header.frameType = bytes2uint16(12);
  header.seqNum = bytes2uint16(14);
  header.command.packed = bytes2uint32(16);

  size_t myOffset;
  // not all return packets have an address; if-block on command type and
  // whether it is an acknowledge
...
vector<uint8_t> APS2EthernetPacket::serialize() const {
  ...
  std::copy(header.dest.addr.begin(), header.dest.addr.end(), insertPt);
  insertPt += 6;
  std::copy(header.src.addr.begin(), header.src.addr.end(), insertPt);
  insertPt += 6;
  ...
  std::copy(start, start + 2, insertPt);
  insertPt += 2;
  ...
  std::copy(start, start + 4, insertPt);
  insertPt += 4;
...
size_t APS2EthernetPacket::numBytes() const {
  size_t trueSize = needs_address(APS_COMMANDS(header.command.cmd))
                        ? NUM_HEADER_BYTES + 4 * payload.size()
                        : NUM_HEADER_BYTES - 4 + 4 * payload.size();
  return std::max(trueSize, static_cast<size_t>(64));
}

APS2EthernetPacket APS2EthernetPacket::create_broadcast_packet() {
  /*
   * Helper function to put together a broadcast status packet that all APS
   * units should respond to.
   */
  APS2EthernetPacket myPacket;

  // Put the broadcast FF:FF:FF:FF:FF:FF in the MAC destination address
  std::fill(myPacket.header.dest.addr.begin(), myPacket.header.dest.addr.end(),
            0xFF);
...
APS2EthernetPacket::chunk(uint32_t addr, const vector<uint32_t> &data,
                          APS2Command cmd) {
  // Break the data up into ethernet frame sized chunks with APS2EthernetPacket
  // headers
  // ethernet frame payload = 1500bytes - 20bytes IPV4 and 8 bytes UDP and 24
  // bytes APS header (with address field) = 1448bytes = 362 words
  // for unknown reasons, we see occasional failures when using packets that
  // large. 256 seems to be more stable.
  ...
  do {
    ...
    std::copy(idx, idx + newPacket.header.command.cnt,
              std::back_inserter(newPacket.payload));
