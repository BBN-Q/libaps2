...
vector<string> enumerate_slices() {
  ...
  */
  ...
  cout << concol::CYAN << numDevices << " APS2 slices"
  ...
  for (unsigned ct = 0; ct < numDevices; ct++) {
    ...
    cout << concol::CYAN << "Device " << ct + 1
         << " IP address: " << ip_addr_buffers[ct] << concol::RESET << endl;
  }
...
vector<string> get_device_ids() {
  /*
  Helper function than enumerates and asks for which (potentially "all") APS to
  talk to.
  */
  vector<string> ip_addrs = enumerate_slices();
  if (ip_addrs.size() == 0) {
  ...
  if (ip_addrs.size() > 1) {
    cout << concol::YELLOW
         << "Choose device ID (or \"all\") [1]: " << concol::RESET;
    string input = "";
    getline(cin, input);
