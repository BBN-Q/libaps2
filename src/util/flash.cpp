...
const option::Descriptor usage[] = {
    ...
    {IP_ADDR, 0, "", "IP", option::Arg::None,
     "	--IP	\tProgram a new IP address."},
    {MAC_ADDR, 0, "", "MAC", option::Arg::None,
     "	--MAC	\tProgram a new MAC address."},
    {DHCP_ENABLE, 0, "", "DHCP", option::Arg::None,
     "	--DHCP	\tProgram DHCP Enable"},
    {SPI, 0, "", "SPI", option::Arg::None,
     "	--SPI	\tWrite the SPI startup sequence."},
    {LOG_LEVEL, 0, "", "logLevel", option::Arg::Numeric,
     "	--logLevel	\t(optional) Logging level level to print to console "
     "(optional; default=2/INFO)."},
    {RESET, 0, "", "reset", option::Arg::None,
     "	--reset	\t(optional) Issue soft reset to FPGA (optional; default=N)."},
...
uint64_t get_mac_input() {
  cout << concol::YELLOW
       << "New MAC address [ENTER to skip]: " << concol::RESET;
  string input = "";
  getline(cin, input);
...
bool get_dhcp_input() {
  cout << concol::YELLOW
       << "Do you want to enable DHCP [y/N]: " << concol::RESET;
  return get_yes_no_input();
}

bool spi_prompt() {
  cout << concol::YELLOW
       << "Do you want to program the SPI startup sequence? [y/N]: "
       << concol::RESET;
  return get_yes_no_input();
}
...
bool get_reset_input() {
  cout << concol::YELLOW
       << "Do you want to reset FPGA [y/N]: " << concol::RESET;
  return get_yes_no_input();
}
...
int main(int argc, char *argv[]) {
  ...
  if (options[MAC_ADDR] || options[IP_ADDR] || interactiveMode) {
    cout << concol::CYAN << "Programmed MAC and IP address at 0x00FF0000 are "
         << concol::RESET << endl;
    cout << concol::CYAN
         << "MAC addr: " << hexn<12> << get_mac_addr(deviceSerial.c_str())
         << concol::RESET << endl;
    char curIP[16];
  ...
  if (options[MAC_ADDR] || interactiveMode) {
    ...
    if (mac_addr != 0) {
      cout << concol::CYAN << "Writing new MAC address" << concol::RESET
           << endl;
  ...
  if (options[SPI] || interactiveMode) {
    ...
    cout << concol::CYAN << "Programmed setup SPI sequence:" << concol::RESET
         << endl;
    ...
    if (runSPI) {
      ...
      cout << concol::CYAN << "Writing SPI startup sequence" << concol::RESET
           << endl;
  ...
  if (options[RESET] || interactiveMode) {
    ...
    if (reset_requested)
      reset(deviceSerial.c_str(), RECONFIG_EPROM_USER);
  }
