...
int main (int argc, char* argv[])
{
	...
	for (auto ip_addr : ip_addrs) {
		...
		auto thread_future = std::async(std::launch::async, [ip_addr, bitfile, target_addr, target_media]() {
			return write_bitfile(ip_addr.c_str(), bitfile.c_str(), target_addr, target_media);
		});
		...
		while (get_bitfile_writing_task(ip_addr.c_str()) != DONE) {
			...
			std::map<APS2_BITFILE_WRITING_TASK, string> task_string_map { {ERASING, "Erasing: "}, {WRITING, "Writing: "}, {VALIDATING, "Validating: "}};
			if ( (cur_bitfile_writing_task == ERASING) || (cur_bitfile_writing_task == WRITING) || (cur_bitfile_writing_task == VALIDATING) ) {
				progress_bar(task_string_map[cur_bitfile_writing_task], get_flash_progress(ip_addr.c_str()));
			}
